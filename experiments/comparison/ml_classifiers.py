...
def load_cwru_dataset(config):
    ...
    return np.array(all_features), np.array(all_labels), fs
...
def run_classification_comparison(X, y, dataset_name, output_dir):
    """在给定数据集上运行分类器对比"""
    ...
    if results_list:
        ...
        fig.savefig(output_dir / f"{dataset_name}_ml_comparison.png", dpi=200)
        plt.close(fig)
...
def main():
    ...
    output_dir = Path("outputs/ml_comparison")
    output_dir.mkdir(parents=True, exist_ok=True)

    logger.info("=" * 60)
    logger.info("ML 分类器对比实验")
    logger.info("=" * 60)
    ...
    X_cwru, y_cwru, fs_cwru = load_cwru_dataset(cwru_config)

    if len(X_cwru) > 0:
        cwru_results = run_classification_comparison(
            X_cwru, y_cwru, "CWRU", output_dir,
        )
    ...
    X_phm, y_phm, fs_phm = load_phm_dataset(phm_config)

    if len(X_phm) > 0:
        phm_results = run_classification_comparison(
            X_phm, y_phm, "PHM2010", output_dir,
        )
