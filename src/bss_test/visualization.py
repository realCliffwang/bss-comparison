"""
BSS evaluation visualization.

Academic-style plotting functions for BSS results:
...
- Comparison bar charts and confusion matrices
- TFA x BSS cross-comparison grids
"""
...
from bss_test.metrics import compute_independence_metric, compute_fault_detection_score
...
def setup_academic_style():
    """Apply academic paper style to all subsequent plots (idempotent)."""
...
def plot_tfa_bss_cross_comparison(cross_results, fs, fault_freqs=None,
                                    title_prefix=""):
    """
    TFA x BSS cross-comparison grid: each cell shows envelope spectrum.
    ...
    fig.suptitle(f"{title_prefix}TFA x BSS Cross-Comparison — Envelope Spectra",
...
def plot_separation_quality_report(cross_results, fs, fault_freqs=None,
    ...
    fig.suptitle(f"{title_prefix}Separation Quality Report — TFA x BSS Cross-Validation",
